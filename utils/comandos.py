...
def btn_secciones_click(seccion):
    ...
    config.imagen = GM.cargar_imagen(canvas_frame=config.lienzo,nombre_imagen="fondos/fondo_index.png",pos_centro_x=1280/2,pos_centro_y=720/2)
    GM.destruir_widget(config.button_agregar)
    GM.destruir_widget(config.button_editar)
    GM.destruir_widget(config.button_eliminar)
    
    if seccion == 'inicio':
        print("Estas en|- Seccion ->",seccion) #
...
def callback_clic_registro(entidad,registro):
    ...
    print("Seccion Actual", config.seccion_actual)
    print("Entidad Seleccionada", config.registro_seleccionado_entidad)
...
def crear_botones_acciones(padre):
    def ventana(seleccion):
        print("creando ventana",seleccion)
        GM.destruir_widget(config.ventana_emergente)
        config.ventana_emergente=None
        if seleccion=="agregar":
            config.ventana_emergente = GM.crear_ventana_emergente(config.ventana_principal,ancho_ventana=350,alto_ventana=400)
            config.lienzo_emergente = GM.crear_lienzo_canvas(config.ventana_emergente,350,400,bg_color="blue")
            ...
            if config.seccion_actual==config.registro_seleccionado_entidad:
                ...
                print("se elimina el id:",id_eliminar)
