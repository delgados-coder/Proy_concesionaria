...
def crear_lienzo_canvas(ventana_padre,ancho,alto,bg_color="#A4A6A6"):
    frame_canvas = Canvas(
    ventana_padre,
    bg = bg_color,
    height = alto,
...
def cargar_imagen(canvas_frame, nombre_imagen, pos_centro_x=0, pos_centro_y=0):
    ...
    if os.path.isfile(imagen_path):
        ...
        canvas_frame.after(1000, lambda: cargar_imagen(canvas_frame, nombre_imagen, medida1, medida2))
...
def crear_tabla(entidad, padre, nombres_de_campos, nombres_de_campos_a_mostrar, pos_x=0, pos_y=0, ancho_tabla=1000, alto_tabla=500):
    ...
    style = ttk.Style()
    style.configure("mystyle.Treeview", highlightthickness=5, bd=3, font=('Arial', 10))  
    style.configure("mystyle.Treeview", rowheight=25, bordercolor="black", borderwidth=2)
    style.map("mystyle.Treeview", background=[('selected', 'black')], foreground=[('selected', 'white')])  
...
#---------------------------------------------------------------------------------------#
def crear_formulario(padre, accion, seccion, pos_x=0, pos_y=0, dict_datos=None):
    campos_clie = ["id_cliente", "nombre", "apellido", "documento", "direccion", "telefono", "correo_electronico"]
    campos_vehi = ["id_vehiculo","dominio","marca","modelo","anio","kilometraje","precio_compra","precio_venta","estado"]
    campos_tran = ["id_transaccion","id_vehiculo","id_cliente","tipo_transaccion","fecha","monto","observaciones"]
    ...
    for i, campo in enumerate(campos):
        # label = tk.Label(padre, text=campo)
        # label.pack()
    ...
    print(campos)
    aceptar_btn = tk.Button(padre, text="Aceptar", command=lambda: aceptar(entries, accion, seccion))
    aceptar_btn.pack()
